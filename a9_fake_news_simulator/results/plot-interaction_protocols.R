...
returned_things <- do_stuff("C:/Users/Anton Laukemper/Desktop/Uni/WS1920/Multi-Agent System Modeling/dmas/a9_fake_news_simulator/results/data/ANY_RANDOM_DISCUSSION", "DISCUSSION")
...
returned_things <- do_stuff("C:/Users/Anton Laukemper/Desktop/Uni/WS1920/Multi-Agent System Modeling/dmas/a9_fake_news_simulator/results/data/ANY_RANDOM_MAJORITY", "MAJORITY OPINION")
...
returned_things <- do_stuff("C:/Users/Anton Laukemper/Desktop/Uni/WS1920/Multi-Agent System Modeling/dmas/a9_fake_news_simulator/results/data/ANY_RANDOM_SIMPLE", "SIMPLE")
...
title = expression(paste("timestep / 100 (log-scale)"))

p <- ggplot(summarydf, aes(x=variable, y=value, color=Opinion)) + 
  geom_line() +
  geom_point() +
  geom_errorbar(aes(ymin=value-sd, ymax=value+sd)) +
  scale_x_continuous(breaks = round(seq(0, 300, by = 10))) +
  scale_y_continuous(breaks = round(seq(0, 100, by = 10))) +
  geom_vline(xintercept = avgStopDis1, size = 1.5, color = "#ABEBC6") +
  # geom_vline(xintercept = avgStopDis1 - sdStopDis1, linetype="longdash", size = 1) +
  # geom_vline(xintercept = avgStopDis1 + sdStopDis1, linetype="longdash", size = 1) +
  geom_vline(xintercept = avgStopDis2, size = 1.5, alpha = 0.4, color = "#2ECC71") +
  # geom_vline(xintercept = avgStopDis2 - sdStopDis2, linetype="longdash", size = 1, alpha = 0.4) +
  # geom_vline(xintercept = avgStopDis2 + sdStopDis2, linetype="longdash", size = 1, alpha = 0.4) +
  geom_vline(xintercept = avgStopDis3, size = 1.5, alpha = 0.1, color = "#336600") +
