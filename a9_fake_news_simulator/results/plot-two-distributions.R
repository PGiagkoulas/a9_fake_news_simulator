...
setwd("/Users/sharbatc/Desktop/msc-project-master/dmas/DMAS/SYO_CLUSTER_SIMPLE")
...
meltDis1$Combination <- "With SYO"

setwd("/Users/sharbatc/Desktop/msc-project-master/dmas/DMAS/ANY_CLUSTER_SIMPLE")
...
meltDis2$Combination <- "Without SYO"

meltAll <- rbind(meltDis2, meltDis1)

meltAll$Combination <- as.factor(meltAll$Combination)
meltAll$Opinion <- as.factor(meltAll$Opinion)

summarydf <- summarySE(meltAll,
                       measurevar="value",
                       groupvars=c("variable", "Opinion", "Combination"))
...
p <- ggplot(summarydf, aes(x=variable, y=value, color=Opinion, alpha=Combination)) + 
  geom_line() +
  #geom_point(size = 0.5) +
  ...
  geom_vline(xintercept = avgStop - sdStop, linetype="longdash", size = 1) +
  geom_vline(xintercept = avgStop + sdStop, linetype="longdash", size = 1) +
  geom_vline(xintercept = oldavgStop, size = 1.2, alpha = 0.2) +
  geom_vline(xintercept = oldavgStop - oldsdStop, linetype="longdash", size = 1, alpha = 0.2) +
  geom_vline(xintercept = oldavgStop + oldsdStop, linetype="longdash", size = 1, alpha = 0.2) +
